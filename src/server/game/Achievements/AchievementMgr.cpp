...
void AchievementMgr::LoadFromDB(PreparedQueryResult achievementResult, PreparedQueryResult criteriaResult)
{
    ...
    {
        for (auto const& fields : *achievementResult)
        {
...
void AchievementMgr::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    else if (GetPlayer()->IsInWorld())
    {
        ...
        TypeContainerVisitor<Warhead::PlayerDistWorker<Warhead::LocalizedPacketDo<Warhead::BroadcastTextBuilder>>, WorldTypeMapContainer > message(_worker);
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    LOG_INFO("server.loading", ">> Loaded {} completed achievements in {}", _allCompletedAchievements.size(), sw);
    LOG_INFO("server.loading", " ");
}
