...
public:
    ...
    static bool HandleNpcSetModelCommand(ChatHandler* handler, uint32 displayId)
    {
        ...
        {
            handler->PSendSysMessage(LANG_COMMAND_FACTION_INVPARAM, Warhead::ToString(displayId));
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    */
    ...
    static bool HandleNpcSetLinkCommand(ChatHandler* handler, ObjectGuid::LowType linkguid)
    {
        ...
        {
            handler->PSendSysMessage("Selected creature {} isn't in creature table", creature->GetGUID().ToString());
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->PSendSysMessage("LinkGUID '{}' added to creature with SpawnId: '{}'", linkguid, creature->GetSpawnId());
        return true;
    }
};
